...
支援的工作表依序為 `Invoice`、`btb411w_xls1`、`allowance`。第一列的第一欄不可為空，且必須包含精確欄名 `最後異動時間` 與 `買方統一編號`。資料列中的單號或日期無法解析時，會跳過整個檔案並顯示原因。

統一編號逐列取自 Excel 的 `買方統一編號`，不使用檔案名稱或資料夾路徑判斷。門檻後異動列只接受 8 碼數字或 `0000000000`，並保留 Excel 數字格式顯示的前導零；若值為空或格式不符，整個工作簿會列為略過。
